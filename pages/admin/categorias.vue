<template>
  <AdminLayout>
    <div class="space-y-6">
      ...
      <!-- Categories Grid -->
      <div v-if="loading && categories.length === 0" class="flex justify-center py-12">
      ...
      <div v-else class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          ...
          <div class="p-6 pb-4">
            <div class="flex items-start justify-between mb-3">
              <div class="flex-1">
                ...
                <p class="text-gray-600 text-sm mb-2">{{ category.description || 'Sem descrição' }}</p>
                <div class="flex items-center gap-2 text-xs text-gray-500">
                  <span class="font-mono bg-gray-100 px-2 py-1 rounded">/categoria/{{ category.slug }}</span>
                </div>
              </div>
            ...
            <div class="flex items-center justify-between">
              <div class="flex items-center gap-4 text-sm">
                ...
                <span class="text-gray-500">
                  <span class="font-medium text-gray-700">{{ category.product_count || 0 }}</span> produtos
                </span>
              </div>
        ...
        <!-- Empty State -->
        <div v-if="categories.length === 0" class="col-span-full text-center py-12 bg-white rounded-xl border-2 border-dashed border-gray-300">
...
const saveCategory = async (categoryData) => {
  await categoriesStore.fetchCategories(); // Recarregar lista após salvar
  closeModal();
};
...
const toggleCategoryStatus = async (category) => {
  try {
    ...
    await categoriesStore.fetchCategories(); // Recarregar lista
    message.value = `Categoria ${category.is_active ? 'desativada' : 'ativada'} com sucesso!`;
...
const deleteCategory = async (category) => {
  if (confirm(`Tem certeza que deseja excluir a categoria "${category.name}"? Esta ação não pode ser desfeita.`)) {
    try {
      ...
      message.value = 'Erro ao excluir categoria. Verifique se não há produtos vinculados.';
      messageType.value = 'error';
    }
  }
};
